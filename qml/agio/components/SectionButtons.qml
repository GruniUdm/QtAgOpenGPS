...
Rectangle {
    ...
    property int numSections: (AgIOService.setTool_isSectionsNotZones ? AgIOService.setVehicle_numSections : AgIOService.setTool_zones[0])
    ...
    function setColors() {
        ...
        if (AgIOService.setDisplay_isDayMode) {
    ...
    function setSectionState (sectionNo: int, new_state: int) {
        ...
        if (settings.setTool_isSectionsNotZones) {
            ...
            else zone_left = AgIOService.setTool_zones[sectionNo]

            //get the right-most section in the zone
            zone_right = AgIOService.setTool_zones[sectionNo+1]
    ...
    function setAllSectionsToState(new_state: int) {
        ...
        if (AgIOService.setTool_isSectionsNotZones) {
            //1:1 correlation between buttons and sections
            for (i=0; i < 65 ; i++) {
                if (i < numSections)
            ...
            for (i=0; i < numSections; i++) {
                ...
                //get the left-most section in the zone
                if (i===0) zone_left = 0
                else zone_left = AgIOService.setTool_zones[i]

                //get the right-most section in the zone
                zone_right = AgIOService.setTool_zones[i+1]
