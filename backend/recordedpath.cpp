                                              ...
                                              )
{
    ...
    GetDubinsPath(*CVehicle::instance(), goal, yt);
    ...
    isEndOfTheRecLine = false;
    //currentPositonIndex = 0;
    set_isDrivingRecordedPath(true);
    return true;
}

void RecordedPath::UpdatePosition(const CYouTurn &yt, bool isBtnAutoSteerOn)
{
...
void RecordedPath::StopDrivingRecordedPath()
{
    isFollowingDubinsHome = false;
    isFollowingRecPath = false;
    isFollowingDubinsToPath = false;
    ...
    set_isDrivingRecordedPath(false);
...
void RecordedPath::GetDubinsPath(CVehicle &vehicle, Vec3 goal, const CYouTurn &yt)
{
    ...
    {
        shortestDubinsList.insert(0, CVehicle::instance()->pivotAxlePos);
...
void RecordedPath::PurePursuitRecPath(CVehicle &vehicle, int ptCount)
{
    ...
    if (CVehicle::instance()->purePursuitIntegralGain != 0)
    {
...

        {
            ...
            {
                inty += pivotDistanceError * CVehicle::instance()->purePursuitIntegralGain * -0.04;
            }
            else
            {
                if (fabs(distanceFromCurrentLinePivot) > 0.02)
                {
                    inty += pivotDistanceError * CVehicle::instance()->purePursuitIntegralGain * -0.02;
    ...
    steerAngleRP = glm::toDegrees(atan(2 * (((goalPointRP.easting - pivotAxlePosRP.easting) * cos(localHeading))
                                                + ((goalPointRP.northing - pivotAxlePosRP.northing) * sin(localHeading))) * CVehicle::instance()->wheelbase / goalPointDistanceSquared));
...
void RecordedPath::PurePursuitDubins(CVehicle &vehicle, const CYouTurn &yt, bool isBtnAutoSteerOn, int ptCount)
{
    double dist, dx, dz;
    ...
    // double angVel = glm::twoPI * 0.277777 * CVehicle::instance()->pn.speed * (tan(glm::toRadians(steerAngleRP))) / CVehicle::instance()->CVehicle::instance()->wheelbase;

    //clamp the steering angle to not exceed safe angular velocity
    //if (fabs(angVel) > mf.CVehicle::instance()->maxAngularVelocity)
    //{
    //    steerAngleRP = glm::toDegrees(steerAngleRP > 0 ?
    //            (atan((mf.CVehicle::instance()->wheelbase * mf.CVehicle::instance()->maxAngularVelocity) / (glm::twoPI * mf.avgSpeed * 0.277777)))
    //        : (atan((mf.CVehicle::instance()->wheelbase * -mf.CVehicle::instance()->maxAngularVelocity) / (glm::twoPI * mf.avgSpeed * 0.277777))));
    //}
...
void RecordedPath::DrawRecordedLine(QOpenGLFunctions *gl, const QMatrix4x4 &mvp)
{
...
void RecordedPath::DrawDubins(QOpenGLFunctions *gl, const QMatrix4x4 &mvp)
{
...
void RecordedPath::updateInterface()
{
    RecordedPathProperties *props = m_recordedPathProperties;

    // Recorded trail line
    QVector<QVector3D> trail;
    int ptCount = recList.size();
    if (ptCount > 0) {
        trail.reserve(ptCount);
        for (int i = 0; i < ptCount; i++)
            trail.append(QVector3D(recList[i].easting, recList[i].northing, 0));
    }
    props->set_recordedLine(trail);

    // Dubins approach path
    QVector<QVector3D> dubins;
    int dubCount = shuttleDubinsList.size();
    if (dubCount > 0) {
        dubins.reserve(dubCount);
        for (int i = 0; i < dubCount; i++)
            dubins.append(QVector3D(shuttleDubinsList[i].easting, shuttleDubinsList[i].northing, 0));
    }
