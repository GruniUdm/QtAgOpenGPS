...
Q_LOGGING_CATEGORY(thicklinecolorsmaterial_log, "thicklinematerial.qtagopengps")

// ============================================================================
// ThickLineColorsMaterial
// ============================================================================

ThickLineColorsMaterial::ThickLineColorsMaterial()
{
    setFlag(Blending);  // Support transparent lines
}

QSGMaterialType *ThickLineColorsMaterial::type() const
{
    static QSGMaterialType type;
    return &type;
}

QSGMaterialShader *ThickLineColorsMaterial::createShader(QSGRendererInterface::RenderMode) const
{
    return new ThickLineColorsMaterialShader();
}

int ThickLineColorsMaterial::compare(const QSGMaterial *other) const
{
    const auto *o = static_cast<const ThickLineColorsMaterial *>(other);
...
void ThickLineColorsMaterial::setColor(const QColor &color)
{
    m_color = color;
}

void ThickLineColorsMaterial::setLineWidth(float width)
{
    m_lineWidth = width;
}

const QSGGeometry::AttributeSet &ThickLineColorsMaterial::attributes()
{
...
ThickLineColorsMaterialShader::ThickLineColorsMaterialShader()
{
    setShaderFileName(VertexStage, QLatin1String(":/AOG/shaders/thicklinecolors.vert.qsb"));
    setShaderFileName(FragmentStage, QLatin1String(":/AOG/shaders/thicklinecolors.frag.qsb"));
}

bool ThickLineColorsMaterialShader::updateUniformData(RenderState &state,
                                                 ...
                                                 QSGMaterial *oldMaterial)
{
    ...
    auto *material = static_cast<ThickLineColorsMaterial *>(newMaterial);

    // Uniform buffer layout (std140):
    // offset 0:  mat4 full mvpMatrix (64 bytes)
    // offset 64: vec2 viewPort (8 bytes)
    // offset 72: float lineWidth (4 bytes, padded to 16)
    ...
    // Update viewport size (offset 64)
